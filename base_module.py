...
class BaseModule(object):
    ...
    def fit(self, train_data, eval_data=None, eval_metric='acc',
            ...
            validation_metric=None, monitor=None, sparse_row_id_fn=None, instrument=False,
            de_profile_data=None, de_filename=None):
        ...
        if de_profile_data is None:
            de_profile_data = {}

        tic_start_bind = time.time()
        self.bind(data_shapes=train_data.provide_data, label_shapes=train_data.provide_label,
                  for_training=True, force_rebind=force_rebind)
        # Make sure all operations have completed
        mx.nd.waitall()
        tic_end_bind = time.time()
        de_profile_data['bind_time'] = tic_end_bind - tic_start_bind
        self.logger.debug("Bind time {} secs".format(de_profile_data['bind_time']))

        if monitor is not None:
            self.install_monitor(monitor)
        
        tic_start_init = time.time()
        self.init_params(initializer=initializer, arg_params=arg_params, aux_params=aux_params,
                         allow_missing=allow_missing, force_init=force_init)
        # Make sure all operations have completed
        mx.nd.waitall()
        tic_end_init = time.time()
        de_profile_data['init_param_time'] = tic_end_init - tic_start_init
        self.logger.debug("Init param time {} secs".format(de_profile_data['init_param_time']))
        
        tic_start_init_opt = time.time()
        self.init_optimizer(kvstore=kvstore, optimizer=optimizer,
                            optimizer_params=optimizer_params)
        # Make sure all operations have completed
        mx.nd.waitall()
        tic_end_init_opt = time.time()
        de_profile_data['init_opt_time'] = tic_end_init_opt - tic_start_init_opt
        self.logger.debug("Init opt time {} secs".format(de_profile_data['init_opt_time']))

        tic_start_init_metric = time.time()
        ...
        # Make sure all operations have completed
        mx.nd.waitall()
        tic_end_init_metric = time.time()
        de_profile_data['init_metric_time'] = tic_end_init_metric - tic_start_init_metric
        self.logger.debug("Init metric time {} secs".format(de_profile_data['init_metric_time']))
        ...
        for epoch in range(begin_epoch, num_epoch):
            ...
            while not end_of_batch:
                ...
                tic_start_compute = time.time()
                self.forward_backward(data_batch)
                # Make sure all operations have completed
                mx.nd.waitall()
                tic_end_compute = time.time()
                curr_batch['forward_backward_time'] = tic_end_compute - tic_start_compute
                tot_compute += (tic_end_compute - tic_start_compute)
                self.logger.debug("Batch {} compute time: {} secs".format(nbatch, curr_batch['forward_backward_time']))
                
                tic_start_update = time.time()
                self.update()
                # Make sure all operations have completed
                mx.nd.waitall()
                tic_end_update = time.time()
                curr_batch['update_time'] = tic_end_update - tic_start_update
                tot_update += (tic_end_update - tic_start_update)
                self.logger.debug("Batch {} update time: {} secs".format(nbatch, curr_batch['update_time']))

                tic_start_update_metric = time.time()
                ...
                # Make sure all operations have completed
                mx.nd.waitall()
                tic_end_update_metric = time.time()
                curr_batch['update_metric_time'] = tic_end_update_metric - tic_start_update_metric
                tot_metric_update += (tic_end_update_metric - tic_start_update_metric)
                self.logger.debug("Batch {} update metric time {} secs".format(nbatch, curr_batch['update_metric_time']))

                tic_start_prepare = time.time()
                try:
                ...
                # Make sure all operations have completed
                mx.nd.waitall()
                tic_end_prepare = time.time()
                curr_batch['prepare_time'] = tic_end_prepare - tic_start_prepare
                tot_prepare_time += (tic_end_prepare - tic_start_prepare)
                self.logger.debug("Batch {} prepare time {} secs".format(nbatch, curr_batch['prepare_time'] ))
                
                if monitor is not None:
                    monitor.toc_print()

                tic_start_batch_callbacks = time.time()
                if end_of_batch:
                ...
                # Make sure all operations have completed
                mx.nd.waitall()
                tic_end_batch_callbacks = time.time()
                curr_batch['end_batch_callbacks_time'] = tic_end_batch_callbacks - tic_start_batch_callbacks
                self.logger.debug("End Batch callbacks time {} secs".format(curr_batch['end_batch_callbacks_time']))
                curr_batch['mem']=psutil.virtual_memory()._asdict()
                curr_batch['swap_mem']=psutil.swap_memory()
                batch_data[nbatch] = curr_batch
                nbatch += 1
            
            curr_epoch['batch_data'] = batch_data
            ...
            #############
            self.logger.info('Epoch[%d] Compute Time cost=%.3f', epoch, tot_compute)
            self.logger.info('Epoch[%d] Update Time cost=%.3f', epoch, tot_update)
            self.logger.info('Epoch[%d] Metric Update Time cost=%.3f', epoch, tot_metric_update)
            self.logger.info('Epoch[%d] Prepare Update Time cost=%.3f', epoch, tot_prepare_time)
            self.logger.info('Epoch[%d] Calculated Time cost=%.3f', epoch, tot_compute + tot_update + tot_metric_update + tot_prepare_time)
            ##########

            tic_start_sync_params = time.time()
            ...
            # Make sure all operations have completed
            mx.nd.waitall()
            tic_end_sync_params = time.time()
            curr_epoch['param_sync_time'] = tic_end_sync_params - tic_start_sync_params
            self.logger.info("Epoch {} param sync time is {} secs".format(epoch, curr_epoch['param_sync_time']))
            ...
            mx.nd.waitall()
            toc_validation = time.time()
            valTime = toc_validation - tic_validation
            self.logger.info('Epoch[%d] Validation Time cost=%.3f', epoch, valTime)
            # end of 1 epoch, reset the data-iter for another epoch
            train_data.reset()
            epoch_data[epoch] = curr_epoch
        de_profile_data['epoch_data'] = epoch_data

        if de_filename is not None:
            dirname = os.path.dirname(de_filename)
            if not os.path.exists(dirname):
                os.makedirs(dirname)
            with open(de_filename, 'w') as dump_file:
                json.dump(de_profile_data, dump_file)
    ################################################################################
    # Symbol information
    ################################################################################
    @property
