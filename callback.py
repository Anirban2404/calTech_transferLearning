...
class Speedometer(object):
    ...
    """
    def __init__(self, batch_size, frequent=50, auto_reset=True, de_influxdb=None, de_database_name='deep_edge', de_task_id=None, de_worker_id=None, de_data_samples=None):
        ...
        self.counter = 0
        self.de_database_name = de_database_name
        self.de_task_id = de_task_id
        self.de_worker_id = de_worker_id
        self.de_data_samples = de_data_samples
        self.auto_reset = auto_reset
        self.de_influxdb_ip = None
        self.de_influxdb_port = None
        if de_influxdb is not None:
            (self.de_influxdb_ip, self.de_influxdb_port) = de_influxdb.split(':')
            ...
            if count % self.frequent == 0:
                ...
                if param.eval_metric is not None:
                    ...
                    if self.auto_reset:
                        ...
                        #if not ( (self.de_influxdb_ip is None) or (self.de_influxdb_port is None) or (self.de_database_name is None) or (self.de_task_id is None) or (self.de_worker_id is None) or (self.de_data_samples is None) ):
                         #   self.send_logs_local(param.epoch, count-self.frequent, speed, name_value[0][1])
                    else:
    ...
    def send_logs(self, epoch, count, speed, accuracy):
        COMMAND = os.environ['DE_LOGGER'] + " --address-db {} --port-db {} --database-name {} --task-id {} --worker-id {} --epoch {} --batch {} --accuracy {} --data-samples {} --throughput {}".format(self.de_influxdb_ip,
                self.de_influxdb_port, self.de_database_name, self.de_task_id, self.de_worker_id, epoch, count, accuracy, self.de_data_samples, speed)
        print(COMMAND)
        subprocess.Popen(COMMAND, shell=True)
    
    def create_measurement(self, epoch, count, speed, accuracy):
        measurement  = {
                        "measurement": self.de_task_id,
                        "tags": {"worker": self.de_worker_id},
                        "time": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-4]+"Z",
                        "fields": { "epoch": epoch,"batch": count,"accuracy": accuracy,"throughput": speed}
                        }
        return measurement

    def send_logs_local(self, epoch, count, speed, accuracy):
        # Hacking here and assuming the databse exists
        self.measurement_list.append(self.create_measurement(epoch, count, speed, accuracy))
        if len(self.measurement_list) > 10:
            client = InfluxDBClient(host=self.de_influxdb_ip, port=self.de_influxdb_port)
            client.switch_database('deep_edge')
            client.write_points(self.measurement_list)
            self.measurement_list = list()
