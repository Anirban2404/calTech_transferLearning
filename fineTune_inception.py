...
def fit(symbol, arg_params, aux_params, train, val, batch_size, num_gpus):
    ...
    print("Saving Model..")
    # save model
    checkpoint = mx.callback.do_checkpoint('calTech-inception', period=1)
    toc = time.time()
...
import time
get_model('http://data.mxnet.io/models/imagenet/inception-bn/Inception-BN', 0)
tic = time.time()
sym, arg_params, aux_params = mx.model.load_checkpoint('Inception-BN', 0)
toc = time.time()
print("inception loading time: ", toc-tic, " seconds")
